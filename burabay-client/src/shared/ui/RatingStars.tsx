  ...
  return (
    <div className="flex items-center">
      ...
      {[...Array(fullStars)].map((_, index) => (
        <StarIcon key={`full-${index}`} fill={COLORS.blue200} /> // Задаем оранжевый цвет для полной звезды
      ))}
      ...
      {[...Array(emptyStars)].map((_, index) => (
        <StarIcon key={`empty-${index}`} fill={COLORS.white} />
      ))}
      ...
      <svg width="0" height="0">
        <defs>
          <linearGradient id="halfGradient">
            <stop offset={`${partialStarWidth}%`} stopColor={COLORS.blue200} />
            <stop offset={`${partialStarWidth}%`} stopColor={COLORS.white} />
          </linearGradient>
        </defs>
      </svg>
    </div>
