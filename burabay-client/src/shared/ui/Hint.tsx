...
export const Hint: FC<Props> = function Hint(props) {
  ...
  const textColor = mode == "error" ? COLORS_TEXT.red : COLORS_TEXT.totalBlack

  // Для режима ошибки показываем просто текст красного цвета без фона/бордера.
  const containerClass =
    mode == "error"
      ? `w-full ${props.className || ""}`
      : `rounded-button p-3 w-full ${bgColor} relative ${props.className || ""}`;

  return (
    <div {...props} className={containerClass}>
      <Typography className={`text-${align}`} color={textColor}>
        {title}
      </Typography>
    </div>
  );
