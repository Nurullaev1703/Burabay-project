            ...
            {(clusterer) => (
              <>
                {announcements.map((announcement, index) => {
                  ...
                  const iconSvg = categoryIconSvgMap[categoryName];

                  // Создаем маркер со встроенной SVG иконкой категории
                  const createMarkerSvg = (
                    color: string,
                    iconSvgContent: string | undefined
                  ) => {
                    // Весь маркер 52x52, цветной фон 39x49, круг 28x28, иконка 16x16
                    // Центрируем цветной фон в canvas 52x52: (52-39)/2 = 6.5 по X, 1.5 по Y
                    const iconElement = iconSvgContent
                      ? `<g transform="translate(26, 17.5) scale(0.47) translate(-17, -17)">${iconSvgContent}</g>`
